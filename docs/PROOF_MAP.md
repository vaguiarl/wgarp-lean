...
Entries describe the checked versions; representation choices and exclusions
are recorded in [REVISION_SCOPE.md](REVISION_SCOPE.md).
...
| Lemma 1, finite CMUs | `WGARP.FiniteCMU`: `finiteCMU_continuous`, `finiteCMU_strictlyIncreasingFirst`, `finiteCMU_strictlyDecreasingSecond` | Attained inner and outer extrema, joint continuity, and strict monotonicity in both arguments |
...
| Theorem 1 | `WGARP.TheoremOne`: `statementI_iff_wgarp` through `statementVI_iff_wgarp`, `theorem_one` | The six conditions are each equivalent to WGARP, using finite-coalition and symmetric-matrix/simplex forms for (ii) and (iii), and exposed as five adjacent equivalences in manuscript order |
...
The Appendix A convexity example is not encoded.
